...
p1 = cross_data$A$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  mutate(z=beta/se) %>% ggplot(aes(y=lod,x=pos,group=cell_cycle,color=cell_cycle_f))  + 
  ...
  xlab("Variant position (Mb)")  + ggtitle("BYxRM (Cross A)") + theme_classic() + scale_color_brewer(name="Cell-cycle stage",
                                                                                                     palette="Dark2")  + theme(text=element_text(size=18)) + ylim(c(0,18))

p3 = cross_data$`3004`$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  mutate(z=beta/se) %>% ggplot(aes(y=lod,x=pos,group=cell_cycle,color=cell_cycle_f))  + 
  geom_hline(yintercept = cross_data$B$trans$cell_cycle_threshold)+ geom_point() +geom_line(linewidth=1.5) + 
  
  scale_x_continuous(breaks = c(.5e6,1e6,1.5e6,2e6),labels = function(x)x/1e6) +
  facet_grid(~chrom_short_f,scales="free_x",space="free") +ylab("LOD")  +xlab("Variant position (Mb)")+ ggtitle("CBS2888xYJM981 (Cross C)") + theme_classic() + scale_color_brewer(name="Cell-cycle stage",
                                                                                                                                                                                   palette="Dark2")  + theme(text=element_text(size=18))+ ylim(c(0,18))

p2 = cross_data$B$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  mutate(z=beta/se) %>% ggplot(aes(y=lod,x=pos,group=cell_cycle,color=cell_cycle_f))  + 
  geom_hline(yintercept = cross_data$B$trans$cell_cycle_threshold)+ geom_point() +geom_line(linewidth=1.5) + 
  
  scale_x_continuous(breaks = c(.5e6,1e6,1.5e6,2e6),labels = function(x)x/1e6) +
  facet_grid(~chrom_short_f,scales="free_x",space="free") +ylab("LOD")  +xlab("Variant position (Mb)") + ggtitle("YJM145xYPS163 (Cross B)") + theme_classic() + scale_color_brewer(name="Cell-cycle stage",
                                                                                                                                                                                   palette="Dark2")  + theme(text=element_text(size=18)) + ylim(c(0,18))
...
p4 =cross_data$A$trans$cell_cycle_beta_all %>% filter(marker == m1) %>% ggplot(aes(y=beta,x=cell_cycle_f)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
  ylab("Cell-cycle occupancy effect") + xlab("Cell cycle stage") + theme(text=element_text(size=18)) + ggtitle(expression(italic("MKT1")))

#cross_data$A$trans$cell_cycle_lods
#cross_data$B$trans$cell_cycle_lods
#length(table(cross_data$`3004`$trans$cell_cycle_lods$bin))
...
for(i in 1:length(cross_data$B$trans$cell_cycle_lods)){
  ...
  print(cross_data$B$trans$cell_cycle_beta_all %>% filter(marker == m1) %>% ggplot(aes(y=beta,x=cell_cycle_f)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
    ylab("Cell-cycle occupancy effect") + xlab("Cell cycle stage") + theme(text=element_text(size=18)) + ggtitle(expression(italic("MKT1"))))
...
p6= cross_data$`3004`$trans$cell_cycle_beta_all %>% filter(marker == m3) %>% ggplot(aes(y=beta,x=cell_cycle_f)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
...
p5 = cross_data$B$trans$cell_cycle_beta_all %>% filter(marker == m_gpa) %>% ggplot(aes(y=beta,x=cell_cycle_f)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
