...
p1 = cross_data$A$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  ...
  xlab("Variant position (Mb)")  + ggtitle("BYxRM (A)") + theme_classic() + scale_color_brewer(palette="Dark2")  + theme(text=element_text(size=18)) + ylim(c(0,18))
...
p3 = cross_data$`3004`$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  ...
  scale_x_continuous(breaks = c(.5e6,1e6,1.5e6,2e6),labels = function(x)x/1e6) +
  facet_grid(~chrom_short_f,scales="free_x",space="free") +ylab("LOD")  +xlab("Variant position (Mb)")+ ggtitle("CBS2888xYJM981 (C)") + theme_classic() + scale_color_brewer(palette="Dark2")  + theme(text=element_text(size=18))+ ylim(c(0,18))
...
p2 = cross_data$B$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  ...
  scale_x_continuous(breaks = c(.5e6,1e6,1.5e6,2e6),labels = function(x)x/1e6) +
  facet_grid(~chrom_short_f,scales="free_x",space="free") +ylab("LOD")  +xlab("Variant position (Mb)") + ggtitle("YJM145xYPS163 (B)") + theme_classic() + scale_color_brewer(palette="Dark2")  + theme(text=element_text(size=18)) + ylim(c(0,18))
...
p6= cross_data$`3004`$trans$cell_cycle_beta_all %>% filter(marker == m3) %>% ggplot(aes(y=beta,x=cell_cycle)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
...
p5 = cross_data$B$trans$cell_cycle_beta_all %>% filter(marker == m_gpa) %>% ggplot(aes(y=beta,x=cell_cycle)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
...
attempt_at_plot
#m3= 
ggsave("fig_final/figure5_cell_cycle.svg",width=16,height=12)
