combined_objects$cis_eqtl_ase_only_with_onepot %>% ggplot(aes(x=Beta,y=estimate,color=FDR < 0.05)) + geom_point() + facet_wrap(~cross2,nrow=3) + theme_bw() + theme(text=element_text(size=18)) +
  coord_cartesian(ylim=c(-2,2)) +
  #stat_cor(method = "spearman") +
  geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + xlab("local eQTL effect (one-pot)")  + ylab("allele-specific expression effect") + 
...
ggsave("fig_final//s7.png",bg="white", dpi=300)
ggsave("fig_final//s7.svg", bg="white", dpi=300)
