...
rbind(trans,cis) %>% count(type,has_interaction) %>% group_by(type) %>% summarise(frac=n/n())
...
p3 = cross_data$B$trans$cell_cycle_beta_all %>% filter(chrom != "chrIII") %>%
  ...
  scale_x_continuous(breaks = c(.5e6,1e6,1.5e6,2e6),labels = function(x)x/1e6) +
  facet_grid(~chrom_short_f,scales="free_x",space="free") +ylab("LOD")  +xlab("Variant position (Mb)") + ggtitle("YJM145xYPS163") + theme_classic() + scale_color_brewer(palette="Dark2")  + theme(text=element_text(size=18)) + ylim(c(0,18))
...
p4 =cross_data$A$trans$cell_cycle_beta_all %>% filter(marker == m1) %>% ggplot(aes(y=beta,x=cell_cycle)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
  ylab("Cell-cycle occupancy effect") + xlab("Cell cycle stage") + theme(text=element_text(size=18)) + ggtitle("MKT1")
...
p5= cross_data$`3004`$trans$cell_cycle_beta_all %>% filter(marker == m3) %>% ggplot(aes(y=beta,x=cell_cycle)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=beta - 1.96*se,ymax=beta + 1.96*se),width=.2) + theme_classic() +
  ylab("Cell-cycle occupancy effect") + xlab("Cell cycle stage") + theme(text=element_text(size=18)) + ggtitle("CYR1")


#p1 + p2 + p3 + p4 + p5
library(patchwork)
attempt_at_plot = ((p1/p2/p3) + plot_layout(guides="collect") | (p4/p5)) + plot_layout(widths=c(2,1))
attempt_at_plot
#m3= 
